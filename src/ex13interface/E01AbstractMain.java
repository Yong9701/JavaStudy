...
/*
 추상메서드를 포함하지 않은 추상클래스를 상속하면 오버라이딩은 강제사항이 아닌 선택사항이 된다.
 즉 필요한 경우에만 오버라이딩 하면된다. 현재 오버라이딩 된 메서드는 삭제해도 에러가 발생하지 않는다.
 */
...
/*
 추상클래스2 : 추상메서드를 포함한 클래스. 추상메서드를 멤버로 포함한 클래스는 반드시 abstract로 선언해야한다.
...
/*
 ...
 해당 클래스도 abstract로 선언해야한다. 단, 이경우 해당 클래스로도 인스턴스를 생성할 수 없게된다.
 따라서 인스턴스 생성을 위해서는 부모의 추상메서드를 자식쪽에서 오버라이딩을 필수로 해야한다.
 */
class AbsClassChild2 extends AbsClass2 {
	/*
	 오버라이딩은 부모쪽의 메서드를 가리고, 새로운 기능을 재정의 하는 것이므로 자식 클래스는
	 추상메서드를 포함하지 않는 상태가 된다. 따라서 인스턴스 생성을 할 수 있다.
...
//		AbsClass1 absClass1 = new AbsClass1();
		...
		// 부모쪽의 멤버메서드를 호출할 수 있다. 하지만 오버라이딩 되었으므로 자식쪽의 메서드가 동작하게 된다.
		...
		/*
		 ...
		 static의 기본규칙을 따른다. 인스턴스 생성없이 단지 클래스명으로 접근하여 호출할 수 있다.
		 */
