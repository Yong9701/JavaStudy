...
/*
 ...
 3. 예외 발생지점으로 if문으로 감지한 후 예외 인스턴스를 생성 및 throw한다.
...
public class Ex06DeveloperDefine {
	...
	public static void main(String[] args) {
		...
		try {
			/*
			 ...
			 여기에서 예외처리한다. 즉 메서드를 호출한 지점에서 예외처리 하는 방식이다.
			 */
			int age = readAge();
	...
	public static int readAge() throws AgeErrorException {
		...
		if(inputAge<0) {
			/*
			 나이를 음수로 판단하면 우리가 정의한 예외 인스턴스를 생성한 후 throw한다.
			 */
			AgeErrorException ex = new AgeErrorException();
			throw ex;
			/*
			 예외 인스턴스를 throw 하는 순간 에러가 발생한다. 이는 프로그램의 종료로
			 이어지므로 반드시 예외처리를 해야한다.
