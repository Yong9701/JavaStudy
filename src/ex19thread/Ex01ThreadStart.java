...
/*
 ...
 - 만약 다른 클래스를 상속해야 한다면 Runnable 인터페이스를 구현하여 만들어야하다.
 */
		...
		for(int i=1; i<=100; i++) {
			...
			try {
				/*
				 ...
				 흐름을 주어진 시간만큼 block 상태로 전환시켜 준다.
				 1000부느이 1초 단위로 기술하게 되므로 아래는 0.01초 만큼
				 block시키게 된다.
				 */
				sleep(10);
			}
...
public class Ex01ThreadStart {
	...
	public static void main(String[] args) {
		...
		/*
		 쓰레드 인스턴스를 통한 쓰레드 생성 및 실행. 쓰레드가 정상적으로 생성되면
		 2개의 작업이 병렬적으로 교차하면서 실행된다.
		 */
		st1.start();
		st2.start();
