...
public class E03CharBooleanType {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 만약 "(Double qutation)으로 감싸면 문자열로 인식하므로 String으로 선언해야한다.
		 */
		char ch1 = '가';
		// 문자열이므로 char로 선언할 수 없다. 에러발생
//		char chStr = '가나다라';
		// 따라서 String으로 표현해야 한다.
		String chStr = "가나다라";
		...
		/*
		 아스키코드 : 1byte로 표현할 수 있는 문자로 영문, 숫자를 십진수로 정의한 것을 말한다.
		 	Ex) A => 65, a => 97로 표현된다
		 유니코드 : 1byte로 표현할 수 없는 문자로 영문과 숫자를 제외한 모든 문자(한글, 한자 등)를
		 2byte로 표현한다. 보통 숫자가 크기때문에 16진수로 표현한다.
		 */
		// 메모리에 65로 저장된다.
		char ch2 = 'A';
		// 정수형 변수를 선언 및 초기화한다.
		...
		/*
		 boolean 타입 : true 혹은 false 두가지의 값만 가질 수 있는 자료형으로 산술연산(+, -), 비교연산(<, >=)에서는 사용할 수 없고 논리연산에서만 사용할 수 있다.
		 */
		boolean bn1 = true;
		boolean bn2 = false;
		...
		/*
		 ...
		 ||(논리Or) : 파이프를 사용하고 둘 중 하나만 참이어도 참을 반환한다. 둘다 거짓일 때만 거짓을 반환한다.
		 */
		// T && F => False를 반환
		boolean bn3 = bn1 && bn2;
