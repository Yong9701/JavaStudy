...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		 인스턴스저장 : 저장시 기존에 저장된 동일한 Key값이 존재하면
		...
		/*
		 int 즉 Integer는 String으로 형변환 할 수 없으므로 타입에러가 발생한다.
		 */
		int number = 20;
//		map.put("age", number);
		...
		/*
		 출력하기
		 	1. 키 값을 알고있을때 : get(key값) 으로 출력한다.
		 */
		System.out.println("키 값을 알때:" + map.get("name"));
		
		/*
		 	2. 키 값을 모를때(혹은 너무 많아서 하나씩 명시하기 힘들떄)
		...
		/*
		 	3. 이터레이터를 통한 출력 : 이터레이터를 통해 출력할떄도 keySet()을 통해
		...
		System.out.println("[age키 값을 삭제후 출력]");
		for(String key : keys) {
