...
public class Ex03ArrayList1 {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 ArrayList와 나머지 컬렉션들은 데이터를 저장하는 내부적인 자료구조만 다를뿐 사용법은
		 100% 동일하다.
		 */
		...
		/*
		 인스턴스저장 : add()
		 	...
		 	add(인덱스, 인스턴스) : 데이터 저장시 인덱스를 직접 부여한다.
		 		...
		 		만약 지정한 인덱스에 이미 저장된 인스턴스가 있다면 삽입(끼워넣기)이 된다.
		 */
		list.add("소녀시대");
		list.add("빅뱅");
		list.add("트와이스");
...
//		list.add(5,"소방차ㅋㅋ"); // 런타임 에러 발생
		...
		/*
		 중복저장 : List는 배열의 특성을 가지므로 데이터를 중복저장 할 수 있다.
		 	동일한 데이터라도 index로 구분할 수 있기때문이다. add()는 인스턴스 추가에 성공할 경우
		 	true를 반환한다.
		 */
		...
		/*
		 ...
		 방법1 : 일반for문 사용. 이때는 인덱스를 통해 접근해야 하므로 get() 메서드를 사용한다.
		 */
		System.out.println("\n[일반for문 사용]");
		...
		/*
		 방법2 : 확장 for문의 경우 인덱스값이 전체 원소를 반복해주므로
		 	좀 더 간단히 표현이 가능하다. 가장 많이 사용되는 방식이다.
		 */
		System.out.println("\n[확장for문 사용]");
		for(String obj : list) {
		...
		/*
		 포함여부확인
		 	contains(인스턴스) : 특정 인스턴스가 저장되어 있는지 확인할때 사용한다.
		 		존재하면 true를 반환한다.
		 */
		...
		/*
		 ...
		 방법1 : 인덱스로 삭제
		 	remove(인덱스) : 인덱스를 통해 삭제를 진행한 후 삭제가 완료되면 해당 인스턴스를 반환한다.
		 			삭제후에는 인덱스가 자동으로 부여된다. 삭제 성공시 해당 인스턴스의 참조값을 반환하므로
		 			삭제 즉시 확인할 수 있다.
		 */
		...
		/*
		 방법3 : 인스턴스의 참조값을 통해 삭제한다. 이 경우에는 삭제에 성공했을때 true를 반환한다.
