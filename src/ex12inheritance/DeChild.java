...
//		System.out.println(name + "이(가) 공부한다.");
	...
	/*
	 오버라이딩 아님 : 부모클래스에서  private 멤버로 선언했으므로 자식쪽에서는 보이지 않아
	 	오버라이딩 대상이 될 수 없다. 해당 메서드는 자식쪽에서 새롭게 확장한 메서드가 된다.
	 */
	public void eat() {
	...
	/*
	 메서드 오버라이딩 조건
	 1. 부모클래스에서 선언될때와 비교해서 접근범위가 동일하거나 그보다 넓으면 가능하다.
	...
	/*
	 static(정적) 메서드는 오버라이딩의 대상이 될 수 없다. 클래스 외부(메서드영역)에 별도로 로드되므로
	 오버라이딩이 되지않고 호출시 각 클래스명을 통해 접근할 수 있다.
	 */
