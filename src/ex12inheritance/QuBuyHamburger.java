...
/*
 문제5] 햄버거를 추상화한 클래스와 기본가격, 세트가격을 추상화한 클래스를 정의하여 햄버거를 주문시 기본가격 혹은 세트가격으로 출력할수 있도록 프로그램을 구현하시오. 
		조건은 다음과 같다. 
		- 햄버거
...
class HamburgerPrice {
	...
	public void showPrice() {
        ...
        System.out.println("결제금액: " + calculatePrice() + "원");
    }
}
//세트가격을 추상화(기본가격을 상속)
class SetPrice extends HamburgerPrice {	
    ...
    public int calculatePrice() {
        // 세트시 500원 할인
        return super.calculatePrice() - 500;
    }
	//햄버거와 세트결제금액 출력(오버라이딩)	
	@Override
    public void showPrice() {
        burger.showBurgerInfo();
        System.out.println("세트 결제금액: " + calculatePrice() + "원");
    }
}
