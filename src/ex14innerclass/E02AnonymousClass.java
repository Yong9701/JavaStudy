...
/*
 ...
 형식]
 	부모클래스명 참조변수 = new 부코믈래스명() {
 		익명클래스의 실행부;
 		부모클래스의 메서드 오버라이딩();
 	};
 	※ 부모클래스를 상속받아 이름없는 자식클래스를 생성하는 개념으로 이해하면 된다.
 		상속되기때문에 오버라이딩이 가능하다. 즉, {} 안쪽은 "자식"의 영역이다.
 */
// 가수를 추상화한 클래스
class Singer {
	String name;
	...
	/*
	 ...
	 Java에서 정의하는 모든 클래스는 Object클래스를 상속하므로 아래와 같이 오버라이딩이 가능하다.
	 */
	@Override
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 : Singer타입의 변수에 GirlGroup을 상속받은 익명클래스를 생성해서 그 참조값을 할당한다.
		 개념적으로 new GirlGroup() {};
		 	=> 익명 extends GirlGroup과 같다.
		 이름이 없는 인스턴스이므로 부모클래스의 이름을 빌려서 자식의 영역을 만든다고 생각하면된다.
		 */
		...
		/*
		 ...
		 다운캐스팅 자체가 불가능하다. 따라서 익명클래스에서는 멤버의 확장은 거의 사용되지 않는다.
		 대부분 오버라이딩을 목적으로 활용된다.
		 */
//		System.out.println((GirlGroup)s3.mic); // 에러발생됨
