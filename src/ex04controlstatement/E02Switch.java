...
public class E02Switch {
	...
	public static void main(String[] args) {
		...
		/*
		 Scanner 클래스 : 사용자로부터 입력값을 받기위한 클래스로 해당 클래스의 메서드가 실행되면 잠깐 실행이 중지되고 입력을 기다린다.
		 				보통 문자열이나 정수를 입력받기위해 사용된다.
		 */
		Scanner scanner = new Scanner(System.in);
		System.out.print("숫자를 입력하세요");
		// nextInt()는 정수를 입력받기 위한 메서드이므로 문자를 입력하면 예외(에러)가 발생하면서 프로그램이 중지된다.
		int iNum = scanner.nextInt();
		System.out.println("입력한 숫자는: "+iNum);
		
		// 정수를 3으로 나누며 나머지는 0, 1, 2만 생성된다.
		int remain = iNum % 3;
		switch (remain) {
		case 0:
...
//		}
		...
		시나리오] 국,영,수 점수의 평균값을 구하여 학점을 출력하는
			...
			단, switch문으로 작성하시오. 
		 */
		int kor=88, eng=70, math=92;
		/* 평균점수를 10으로 나눈 몫을 통해 점수의 구간을 특정할 수 있다. */
		double avg = (kor+eng+math) / 3.0;
		int avgInt = (int)avg /10;
